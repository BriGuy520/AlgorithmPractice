...
// (maximum sum of distances, integer >= 0), k (number of towns to visit, k >= 1) and ls (list of distances, all distances are positive or zero integers 
// and this list has at least one element). The function returns the "best" sum ie the biggest possible sum of k distances less than or equal to the given 
...
int chooseBestSum(int t, int k, int ls[], int szls) {
    ...
    while(i < j){
        ...
        int total_distance = k[i] + k[j] + k[mid];

        if(total_distance > high && total_distance <= t){
            high = total_distance;
        }
...
int main(){
    ...
    int ys[] = {91, 74, 74, 85, 73, 81, 87};

    int size = sizeof(ys) / sizeof(ys[0]);

    chooseBestSum(230, 3, ys, size);        
}